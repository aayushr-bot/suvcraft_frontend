    ...
    <div className="w-full bg-white min-h-screen">
      <div className="mx-auto w-full max-w-[1440px] px-4 pt-6 pb-10 md:px-8">
        <nav className="text-[13px] text-[#8c8c8c] mb-4">
          ...
          <span className="mx-1.5">›</span>
          <Link href={`/orders/${order.id}`} className="hover:text-ink">ID {order.id}</Link>
          <span className="mx-1.5">›</span>
        ...
        <div className="flex flex-row items-center justify-between gap-3 mb-2">
          <h1 className="text-[24px] md:text-[28px] font-bold text-ink leading-tight">Track Order: {order.id}</h1>
          <Link
        ...
        {/* Timeline */}
        {isCancelled ? (
          <div className="mb-8 rounded-[12px] border border-red-200 bg-red-50 p-5 flex items-center gap-3">
            <div className="text-[28px]">⊘</div>
            <div>
              <h3 className="text-[15px] font-bold text-red-800">Order Cancelled</h3>
              <p className="text-[12px] text-red-700 mt-0.5">There&apos;s nothing to track — this order is no longer active.</p>
            </div>
          </div>
        ) : (
        ...
        {/* Bottom two-column block — payment + delivery + summary */}
